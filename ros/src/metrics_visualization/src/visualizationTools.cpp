...
bool systemDelayInited = false;  // false
double systemTime = 0;
double systemInitTime = 0;
bool systemInited = false;  // false
...
bool createFolderIfNotExist(const std::string &folder) {
  size_t pos = 0;
  std::string dir;
  int status = 0;
  
  // create parent directories recursively
  while((pos = folder.find_first_of("/\\", pos + 1)) != std::string::npos) {
      dir = folder.substr(0, pos);
      if (dir.empty()) continue; // Skip if empty (happens with leading slash)
  ...
  if (status == -1 && errno != EEXIST) {
      ROS_ERROR("Failed to create directory %s: %s", folder.c_str(), strerror(errno));
      return false;
  }
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("saveFolder", saveFolder);
  ...
  nhPrivate.getParam("robotNamespace", robotNamespace);

  string prefix = robotNamespace.empty() ? "" : "/" + robotNamespace;
  string camera_prefix = robotNamespace.empty() ? "" : "/" + robotNamespace + "/camera_ir";

  ros::Subscriber subOdometry = nh.subscribe<nav_msgs::Odometry> (prefix + "/falco_planner/state_estimation", 5, odometryHandler);

  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2> (camera_prefix + "/camera/depth/points", 5, laserCloudHandler);

  ros::Subscriber subRuntime = nh.subscribe<std_msgs::Float32> (prefix + "/data_recording/runtime", 5, runtimeHandler);

  ros::Publisher pubOverallMap = nh.advertise<sensor_msgs::PointCloud2> ("/data_visualize/overall_map", 5);

  ros::Publisher pubExploredArea = nh.advertise<sensor_msgs::PointCloud2> (prefix + "/data_visualize/explored_areas", 5);
  pubExploredAreaPtr = &pubExploredArea;

  ros::Publisher pubTrajectory = nh.advertise<sensor_msgs::PointCloud2> (prefix + "/data_recording/trajectory", 5);
  pubTrajectoryPtr = &pubTrajectory;

  ros::Publisher pubExploredVolume = nh.advertise<std_msgs::Float32> (prefix + "/data_recording/explored_volume", 5);
  pubExploredVolumePtr = &pubExploredVolume;

  ros::Publisher pubTravelingDis = nh.advertise<std_msgs::Float32> (prefix + "/data_recording/traveling_distance", 5);
  pubTravelingDisPtr = &pubTravelingDis;

  ros::Publisher pubTimeDuration = nh.advertise<std_msgs::Float32> (prefix + "/data_recording/time_duration", 5);
  ...
  saveFolder += timeString;
    // Create the directories if a folder path is specified
  if (!saveFolder.empty() && !createFolderIfNotExist(saveFolder)) {
    ROS_ERROR("Failed to create folder: %s", saveFolder.c_str());
    return -1;
  }
  
  metricFile = saveFolder + "/metric.txt";
  trajFile = saveFolder + "/trajectory.txt";     
  ...
  printf("\nExploration metrics and vehicle trajectory are saved in 'src/metrics_visualization/log/%s'.\n\n", robotNamespace.c_str());

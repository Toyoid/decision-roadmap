...
namespace globalPlanner{
	DEP::DEP(const ros::NodeHandle& nh, const std::string& robot_ns) : DRMBase(nh, robot_ns) {
		this->ns_ = "DEP";
		this->hint_ = "[DEP]";
		this->initParam();
		this->initModules();
	...
	void DEP::registerPub(){
		...
		this->candidatePathPub_ = this->nh_.advertise<visualization_msgs::MarkerArray>(this->rostopic_prefix_ + "/dep/candidate_paths", 10);
		...
		// this->bestPathGoalPub_ = this->nh_.advertise<geometry_msgs::PointStamped>(this->rostopic_prefix_ + "/agent/current_goal", 5); 
	}
	...
	void DEP::registerCallback(){
		...
		this->currGoalSub_ = this->nh_.subscribe(this->rostopic_prefix_ + "/agent/current_goal", 5, &DEP::currGoalCB, this); 

		this->navTargetSub_ = this->nh_.subscribe(this->rostopic_prefix_ + "/env/nav_target", 5, &DEP::navTargetArrayCB, this);
	}
	...
	void DEP::buildRoadMap(){
		...
		for (std::shared_ptr<PRM::Node>& n : newNodes){
			...
			for (std::shared_ptr<PRM::Node>& nearestNeighborNode : knn){ // Check collision last if all other conditions are satisfied
				...
				if (n->adjNodes.size() < this->nnNum_ and nearestNeighborNode->adjNodes.size() < this->nnNum_) {  // debug: limit edge number for proper training
...
//		<< endl;
	...
	void DEP::navTargetArrayCB(const decision_roadmap_agent::TargetArray::ConstPtr& navTargets) {
		this->navTargetVec_.clear();
		...
		if (!navTargets->targets.empty()) {
			cout << "[DEP]: " << navTargets->targets.size() << " navigation targets received from target array." << endl;
		}
	}
	...
	visualization_msgs::MarkerArray DEP::buildRoadmapMarkers(){  
		...
		for (std::shared_ptr<PRM::Node> n : this->prmNodeVec_){
			...
			if (n->adjNodes.size() > 20) {
				std::cout << "\033[1;31mNode " << n->pos.transpose() << " has " << n->adjNodes.size() << " neighbors." << "\033[0m" << std::endl;
			}

...
namespace gazebo {
  ...
  void DroneSimpleController::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
  {
    ...
    // Read the robot namespace from SDF
    if (_sdf->HasElement("robot_namespace"))
      robot_ns_ = _sdf->GetElement("robot_namespace")->Get<std::string>();
    
    // Create node handle with namespace only once
    node_handle_ = new ros::NodeHandle(robot_ns_);
    
    // Store model name for debugging
    model_name_ = _model->GetName();
    ROS_INFO("Loading plugin for robot '%s' with namespace '%s'", model_name_.c_str(), robot_ns_.c_str());
    
    // Now configure topics (without namespace prefix since node_handle will add it)
    cmd_normal_topic_ = "cmd_vel";
    cmd_acc_topic_ = "cmd_acc";
    target_pose_topic_ = "setpoint_pose";
    takeoff_topic_ = "takeoff";
    land_topic_ = "land";
    reset_topic_ = "reset";
    posctrl_topic_ = "posctrl";
    gt_pose_topic_ = "pose_raw";
    gt_vel_topic_ = "vel_raw";
    gt_acc_topic_ = "acc_raw";
    gt_odom_topic_ = "odom_raw";
    switch_mode_topic_ = "vel_mode";

    if (!_sdf->HasElement("bodyName"))
    { 
        // Get the base_link specifically instead of any link
        link = _model->GetLink("base_link");
        if (!link) {
            ROS_FATAL("Could not find base_link for model %s", model_name_.c_str());
            return;
        }
        ...
        /* 
        Instead of using world->EntityByName, use GetLink from the model.
        For the multi-robot system, this line of code will load the same link for all the robots:
        link = boost::dynamic_pointer_cast<physics::Link>(world->EntityByName(link_name_));
        */
        link = _model->GetLink(link_name_);
        if (!link) {
            ROS_FATAL("Could not find link %s in model %s", link_name_.c_str(), model_name_.c_str());
            return;
        }
    }

    ROS_INFO("Link of %s loaded, link name: %s", model_name_.c_str(), link_name_.c_str());
    ROS_INFO("[Model: %s]: Model address: %p", model_name_.c_str(), static_cast<void*>(_model.get()));
    ROS_INFO("[Model: %s]: Link address: %p", model_name_.c_str(), static_cast<void*>(link.get()));
  ...
  void DroneSimpleController::UpdateDynamics(double dt){
    ...
    // if (state_topic_.empty())
      ...
      // Debug: print the pose data of each model
      // ROS_INFO("[Model: %s]: Link: %s, Pose: %f, %f, %f", model_name_.c_str(), link_name_.c_str(), pose.Pos().X(), pose.Pos().Y(), pose.Pos().Z());
      // ROS_INFO("[Model: %s]: Model address: %p", model_name_.c_str(), static_cast<void*>(_model.get()));
      // ROS_INFO("[Model: %s]: Link address (before publishing pose): %p", model_name_.c_str(), static_cast<void*>(link.get()));
      // ROS_INFO_STREAM("Model address: " << _model.get());
      // ROS_INFO_STREAM("Link address: " << link.get());
      ...
      gt_pose_stamped.header.frame_id = "map"; // Add namespace to frame id
      ...
      tw_stamped.twist = tw;
      tw_stamped.header.frame_id = "base_link"; // Add namespace
      ...
      tw_acc_stamped.header.frame_id = "base_link"; // Add namespace
      tw_acc_stamped.header.stamp = ros::Time::now();
      pub_gt_acc_.publish(tw_acc_stamped);

      //publish odometry
      nav_msgs::Odometry odom;
      odom.header.frame_id = "map"; // Add namespace
      odom.child_frame_id = "base_link"; // Add namespace

...
namespace AutoFlight{
	flightBase::flightBase(const ros::NodeHandle& nh) : nh_(nh){
		...
		// this->targetPubWorker_ = std::thread(&flightBase::publishTarget, this);  // define and start the "taregt-publish" thread
		// this->targetPubWorker_.detach();  // detach the "taregt-publish" thread (in contrast to this->targetPubWorker_.join())
	...
	flightBase::flightBase(const ros::NodeHandle& nh, const std::string& robot_ns) : nh_(nh){	
		...
		// subscriber
		this->odomSub_ = this->nh_.subscribe(prefix + "/odom", 10, &flightBase::odomCB, this);
		// this->clickSub_ = this->nh_.subscribe("/move_base_simple/goal", 10, &flightBase::clickCB, this);
		

		// publisher
		this->posePub_ = this->nh_.advertise<geometry_msgs::PoseStamped>(prefix + "/setpoint_pose", 10);
		this->statePub_ = this->nh_.advertise<tracking_controller::Target>(prefix + "/autonomous_flight/target_state", 10);
	...
	void flightBase::publishTarget(){
		...
		while (ros::ok()){
	        if (this->poseControl_){
				...
				// cout << "[AutoFlight]: Publishing poseTgt_: " << this->poseTgt_ << endl;
				// directly publish poseTgt to /gazebo node, and usually can only be used for straightforward line-like movement(takeoff, stop)
	        }
	        else{
				this->statePub_.publish(this->stateTgt_);
				// cout << "[AutoFlight]: Publishing stateTgt_: " << this->stateTgt_ << endl;
				// publish the controlling target of the quadcopter state to /tracking_controller, and this is used for general cases
			}
			r.sleep();
		}			
	}
	...
	void flightBase::run(){
		...
		while (ros::ok()){
			...
			// if target is only specified as position, velocity or acceleration, or only two of the three, the control performance is bad

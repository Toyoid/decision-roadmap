...
namespace mapManager{
	...
	occMap::occMap(const ros::NodeHandle& nh, const std::string& robot_ns) : nh_(nh){
		this->ns_ = "occupancy_map";
		this->hint_ = "[OccMap]";
	...
	void occMap::initParam(){
		...
		if (not this->nh_.getParam(this->ns_ + "/depth_image_topic", this->depthTopicName_)){
			...
			prefix = this->robot_ns_ == this->single_robot_ns_? "": "/" + this->robot_ns_ + "/camera_ir";
			this->depthTopicName_ = prefix + this->depthTopicName_;
		...
		if (not this->nh_.getParam(this->ns_ + "/point_cloud_topic", this->pointcloudTopicName_)){
			...
			prefix = this->robot_ns_ == this->single_robot_ns_? "": "/" + this->robot_ns_ + "/camera_ir";
			this->pointcloudTopicName_ = prefix + this->pointcloudTopicName_;
		...
		if (this->localizationMode_ == 0){
			...
			if (not this->nh_.getParam(this->ns_ + "/pose_topic", this->poseTopicName_)){
				...
				prefix = "/" + this->robot_ns_;
				this->poseTopicName_ = prefix + this->poseTopicName_;
		...
		if (this->localizationMode_ == 1){
			...
			if (not this->nh_.getParam(this->ns_ + "/odom_topic", this->odomTopicName_)){
				...
				prefix = "/" + this->robot_ns_;
				this->odomTopicName_ = prefix + this->odomTopicName_;
	...
	void occMap::registerCallback(){
		...
		this->occTimer_ = this->nh_.createTimer(ros::Duration(0.025), &occMap::updateOccupancyCB, this);  // ros::Duration(0.05)

		// map inflation callback
		this->inflateTimer_ = this->nh_.createTimer(ros::Duration(0.025), &occMap::inflateMapCB, this);  // ros::Duration(0.05)
	...
	void occMap::registerPub(){
		...
		this->depthCloudPub_ = this->nh_.advertise<sensor_msgs::PointCloud2>(prefix + this->ns_ + "/depth_cloud", 10);  
		this->mapVisPub_ = this->nh_.advertise<sensor_msgs::PointCloud2>(prefix + this->ns_ + "/voxel_map", 10);
		this->inflatedMapVisPub_ = this->nh_.advertise<sensor_msgs::PointCloud2>(prefix + this->ns_ + "/inflated_voxel_map", 10);
		this->map2DPub_ = this->nh_.advertise<nav_msgs::OccupancyGrid>(prefix + this->ns_ + "/2D_occupancy_map", 10);
		this->mapExploredPub_ = this->nh_.advertise<sensor_msgs::PointCloud2>(prefix + this->ns_+"/explored_voxel_map",10);
		// publish service
		this->collisionCheckServer_ = this->nh_.advertiseService(prefix + this->ns_ + "/check_pos_collision", &occMap::checkCollision, this);
	}

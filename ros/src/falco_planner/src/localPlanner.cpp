...
string depthCloudTopic = "/camera/depth/points";  
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2> (depthCloudTopic, 5, laserCloudHandler);  // namespace specified in launch file

  ros::Subscriber subTrackPoint = nh.subscribe<nav_msgs::Odometry> (ns_prefix + "/falco_planner/track_point_odom", 5, trackPointHandler);   // change to 0 for RL reset

  ros::Subscriber subJoystick = nh.subscribe<sensor_msgs::Joy> (ns_prefix + "/falco_planner/joy", 5, joystickHandler);

  ros::Subscriber subGoal = nh.subscribe<geometry_msgs::PointStamped> (ns_prefix + "/falco_planner/way_point", 5, goalHandler);  // change to 0 for RL reset

  ros::Subscriber subAutoMode = nh.subscribe<std_msgs::Float32> (ns_prefix + "/falco_planner/auto_mode", 5, autoModeHandler);

  ros::Subscriber subClearSurrCloud = nh.subscribe<std_msgs::Empty> (ns_prefix + "/falco_planner/clear_surr_cloud", 5, clearSurrCloudHandler);

  ros::Publisher pubPath = nh.advertise<nav_msgs::Path> (ns_prefix + "/falco_planner/path", 5);  // change to 0 for RL reset
  nav_msgs::Path path;

  #if PLOTPATHSET == 1
  ros::Publisher pubFreePaths = nh.advertise<sensor_msgs::PointCloud2> (ns_prefix + "/falco_planner/free_paths", 2);  

  ros::Publisher pubLaserCloud = nh.advertise<sensor_msgs::PointCloud2> (ns_prefix + "/falco_planner/collision_avoidance_cloud", 2);  
  #endif
      ...
      *laserCloudStack[laserCloudCount] = *laserCloudDwz;
      ...
      while (pathScale >= minPathScale) {
        ...
        if (selectedGroupID >= 0 && (relativeGoalDis > stopDis || relativeGoalX > 0)) {
          ...
          path.header.frame_id = frame_id_prefix + "track_point";
          ...
          freePaths2.header.frame_id = frame_id_prefix + "track_point";

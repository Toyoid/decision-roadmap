...
class InputArgs:
    ...
    episode_start: int = 0
    load_as_init: lambda x: bool(strtobool(x)) = False
    fg_drm_checkpoint_file: str = 'fg_drm_nav/20250309_maze_simple/checkpoint_999.pth'
    dynamic_prm_checkpoint_file: str = 'dynamic_prm_nav/20250321_141526_maze_medium/checkpoint_799.pth'
    # dynamic_prm_checkpoint_file: str = 'drm_nav/checkpoint.pth'
...
def main():
    ...
    if load_as_init:
        # load the a checkpoint as initialization
        if drm_name == "dep":
            checkpoint_file = f"{args.model_path}/{dynamic_prm_checkpoint_file}"
            print(f"[Load Model]: Load Dynamic-PRM model from {checkpoint_file}")
        elif drm_name == "fg_drm":
            checkpoint_file = f"{args.model_path}/{fg_drm_checkpoint_file}"
            print(f"[Load Model]: Load FG-DRM model from {checkpoint_file}")
        else:
            raise ValueError("Unknown DRM name")
        if args.device == 'cpu':
            checkpoint = torch.load(f'{checkpoint_file}', map_location=torch.device('cpu'))
        else:
            checkpoint = torch.load(f'{checkpoint_file}', map_location=args.device)
        actor.load_state_dict(checkpoint['actor_network'])
        qf1.load_state_dict(checkpoint['qf1_network'])
        qf2.load_state_dict(checkpoint['qf2_network'])
    ...
    print(80 * "*")
    global_step = 0
    episode_ita = episode_start  # re-start from episode_start
    ...
    while episode_ita < args.train_num_episodes:
        ...
        while not rospy.is_shutdown():
            ...
            if global_step <= args.learning_starts and not load_as_init:
            ...
            if global_step > args.learning_starts:
                ...
                if args.save_model:
                    if (global_step % args.model_save_frequency == 0) or ((episode_ita == args.train_num_episodes - 1) and done):
                        if drm_name == "fg_drm":
                            save_dir = f"{args.model_path}/fg_drm_nav/{current_time}"
                        elif drm_name == "dep":
                            save_dir = f"{args.model_path}/dynamic_prm_nav/{current_time}"

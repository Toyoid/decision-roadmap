...
class NavUniformGraphEnv(NavDRMEnvBase):
    """
    RL Environment Class for Single-Robot Navigation based on Uniform Graph Decision Roadmap
    """
    ...
    def _get_next_state(self):
        ...
        for node in self.route_node:
            ...
            distances = np.abs(x - node_complex)
            # assume a node has been visited when query distance is lower than 0.8
            index = np.argwhere(distances < 0.8)
            guidepost[index] = 1
        ...
        edge = deepcopy(edges_adj_idx[current_node_idx])
